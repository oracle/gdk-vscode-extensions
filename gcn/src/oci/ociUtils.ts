...
export async function getCompartment(authenticationDetailsProvider: common.ConfigFileAuthenticationDetailsProvider, id : string): Promise<identity.models.Compartment> {
    const client = new identity.IdentityClient({ authenticationDetailsProvider: authenticationDetailsProvider });
    const getCompartmentsRequest: identity.requests.GetCompartmentRequest = {
        compartmentId : id
    };
    return (await client.getCompartment(getCompartmentsRequest)).compartment;
}
