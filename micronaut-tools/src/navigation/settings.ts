...
// Represents a persistent global/workspace string setting, also sets a runtime context value
export class StringSetting {
    ...
    constructor(key: string, defaultValue: string, global: boolean) {
        this.key = key;
        this.value = defaultValue;
        this.global = global;
    }
    ...
    async initialize(context: vscode.ExtensionContext) {
        const memento = this.global ? context.globalState : context.workspaceState;
        const persistedValue: string = memento.get(this.key, String(this.value));
        await this.apply(undefined, persistedValue);
    }
    
    async set(context: vscode.ExtensionContext, value: string) {
        await this.apply(context, value);
    }
    ...
    private async apply(context: vscode.ExtensionContext | undefined, value: string) {
        this.value = value;
        await vscode.commands.executeCommand('setContext', this.key, this.value);
        if (context) {
            const memento = this.global ? context.globalState : context.workspaceState;
