...
export async function createSetupKubernetesPodDeletionStage(authenticationDetailsProvider: common.ConfigFileAuthenticationDetailsProvider, pipelineID: string, commandSpecArtifactID: string, subnetID: string, tags?: { [key:string]: string }): Promise<devops.models.DeployStage> {
    const client = new devops.DevopsClient({ authenticationDetailsProvider: authenticationDetailsProvider });
    const requestDetails: devops.models.CreateShellDeployStageDetails = {
        displayName: 'Pod Deletion',
