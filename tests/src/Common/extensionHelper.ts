...
export async function resolveExtensions(extensionIDs: string[]): Promise<string[]> {
    if (process.env['TEST_EXTENSION_SOURCE'] === 'marketplace')
        return extensionIDs;
...
async function resolveExtension(extensionID: string, urls: ExtensionMap<string>): Promise<string> {
    if (extensionID in urls)
        return await download(extensionID, urls[extensionID as ExtensionName]) ?? extensionID;
    return extensionID;
}
...
async function download(name: string, url?: string): Promise<string | undefined> {
    ...
    if (result.downloadStatus === 'ABORTED')
        return path.join(downloadPath, path.basename(url));
...
function getExtensionNameFromURL(url: string): ExtensionName | undefined {
    for (const ext of (Object.keys(extensionFileNames) as ExtensionName[])) {
        ...
        if (regExp && regExp.test(path.basename(url)))
            return ext;
    }
    return undefined;
}
...
export async function obtainLatestArtifactsURLs(): Promise<ExtensionMap<string>> {
    return mapExtensionURLS((await Promise.all([
        obtainArtifactURLsFromURL("https://ci-builds.apache.org/job/Netbeans/job/netbeans-vscode/lastStableBuild/api/json"),
        obtainArtifactURLsFromURL("https://graalvm.oraclecorp.com/jenkins/job/GCN-vscode-micronaut-main/lastStableBuild/api/json"),
        obtainArtifactURLsFromURL("https://graalvm.oraclecorp.com/jenkins/job/vscode-graalvm-micronaut-master/lastStableBuild/api/json")
...
async function obtainArtifactURLsFromURL(url: string): Promise<string[]> {
    ...
    for (const artifact of json.artifacts) {
        if (artifact.fileName.endsWith('.vsix'))
            out.push(json.url + 'artifact/' + artifact.relativePath);
    }
    return out;
}