 ...
 * @returns
 */
async function waitForContextChange(
  oldValue: string | undefined,
  buildPipeline: BuildPipelineNode,
  timeout: number,
...
suite('Build pipeline Test Suite', function () {
  ...
  const DEPLOY_PROJECT_NAME: string = process.env['TEST_DEPLOY_PROJECT_NAME']
    ? process.env['TEST_DEPLOY_PROJECT_NAME']
    : 'base-oci-template-testpipeline';
  ...
  test('Environment setup', async () => {
    ...
    let DevOpsProjects: any[] = await ociUtils.listDevOpsProjects(provider, COMPARTMENT_OCID);
