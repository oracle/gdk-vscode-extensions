...
function findCacheTime() {
    ...
    fs.readFileSync(path.join(admCacheDir, 'segments')).toString().split('\n').map(s => {
...
suite('ADM Test Suite', function() {
    ...
    test('Project setup', async () => {
        ...
        buildscript = projectType === 'gradle' ? 
        ...
        deployProjectName = process.env["TEST_DEPLOY_PROJECT_NAME"] ? process.env["TEST_DEPLOY_PROJECT_NAME"] : path.basename(projectRoot);
    ...
    test('Locate knowledgebase', async() => {
        ...
        let kbs : adm.models.KnowledgeBaseSummary[] = (await ociUtils.listKnowledgeBases(auth.getProvider(), projectCompartment)).filter(
            kb => kb.displayName === deployProjectName + 'Audits');
    ...
    test('Display while no data exist', async () => {
        ...
        let auditData : any = await vscode.commands.executeCommand('nbls.projectAudit.display', auditedSubprojectUri.toString(), knowledgeBaseOcid, 
            { 
                force : false,
                compute : false,
    ...
    test('Test forced audit update', async () => {
        ...
        let auditData : any = await vscode.commands.executeCommand('nbls.projectAudit.execute', auditedSubprojectUri.toString(), knowledgeBaseOcid, 
            { 
                profile: selectProfile || undefined,
                returnData: true,
                displaySummary: false,
    ...
    test('Check that buildscript change removes an error', async () => {
        ...
        if (projectType === 'gradle') {
            ...
            function removeMavenDependency(dep : string) {
                let x : number = -1;
                lines.filter((s, n) => {
                    if (s.indexOf(`artifactId>${dep}</artifactId`) > 0) { x = n; }
                });
                if (x < 0) {
                    return;
                }
