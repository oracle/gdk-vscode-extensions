...
import {
  ActivityBar, EditorTab, EditorView, SideBarView, TextEditor, ViewContent, ViewControl, ViewItem, ViewSection,
    ...
    describe(`Project ${project.prectName}`, function () {
      ...
      describe("Endpoints", function () {
        ...
        it("Check Endpoints length", async function () {
          ...
          assert.strictEqual(endpointsData.endpointsItems?.length, 3, "Micronaut Tools Endpoints UI doesn't contain expected amount of items.");
        });
        ...
        it("Navigate Endpoints", async function () {
          ...
          for (const endpointText of Object.keys(endpointsData.endpoints)) {
            ...
            assert.strictEqual(activeTitle, 'ProfilePicturesController.java', "Endpoint " + endpointText + " didn't navigate to target.");
            const location = await new TextEditor(editorView).getCoordinates();
            assert.strictEqual(expectedEndpointLocations[endpointText], location[0], "Endpoint " + endpointText + " didn't navigate to correct location.");
      ...
      describe("Beans", function () {
        ...
        it("Check Beans length", async function () {
          ...
          assert.strictEqual(beansData.beansItems?.length, 1, "Micronaut Tools Beans UI doesn't contain expected amount of items.");
        });

        it("Check Beans content", async function () {
          const expectedBeansTexts = ['profilePicturesController'];
          const beansTexts = [];
          for (const beansItem of beansData.beansItems ?? [])
            beansTexts.push(await beansItem.getText());
        ...
        it("Navigate Bean", async function () {
          ...
          for (const beanItem of beansData.beansItems) {
            ...
            assert.strictEqual(activeTitle, 'ProfilePicturesController.java', "Bean " + text + " didn't navigate to target.");
            const location = await new TextEditor(editorView).getCoordinates();
            const expectedBeanLocation = 42;
            assert.strictEqual(expectedBeanLocation, location[0], "Bean " + text + " didn't navigate to correct location.");
            await editorView.closeEditor(activeTitle);
          }
        });
      });
